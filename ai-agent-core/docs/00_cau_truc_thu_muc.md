...
│   │   └── quiz_config.yaml
│   ├── scripts/                # Script chạy huấn luyện
...
1.  **Train:** `training/scripts/` $\rightarrow$ `models/adapters/`.
2.  **Run:** `.NET Backend` $\rightarrow$ `src/api/main.py` $\rightarrow$ `src/core/orchestrator.py`.
3.  **Orchestrator:** `src/core/bkt.py` $\rightarrow$ `src/core/irt.py` $\rightarrow$ `src/adapters/llm_manager.py` $\rightarrow$ `models/adapters/`.
