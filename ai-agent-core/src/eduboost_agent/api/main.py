# src/eduboost_agent/api/main.py
#
...
from eduboost_agent.llm.llm_manager import LLMManager
from eduboost_agent.api.app_state import runtime
from eduboost_agent.api.routes import health as health_routes
from eduboost_agent.api.routes import rag as rag_routes
from eduboost_agent.api.routes import tutor as tutor_routes

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

DEFAULT_FAISS_INDEX_PATH = "var/faiss/faiss_index"
DEFAULT_FAISS_SEED_PATH = "resources/faiss-seed/faiss_index"


def _bootstrap_faiss_seed(index_path: str) -> None:
    """Copy the versioned FAISS seed into an empty runtime volume."""
    seed_path = os.getenv("FAISS_SEED_PATH") or DEFAULT_FAISS_SEED_PATH
    target_files = [Path(f"{index_path}.bin"), Path(f"{index_path}.pkl")]
    seed_files = [Path(f"{seed_path}.bin"), Path(f"{seed_path}.pkl")]
    if any(target.exists() for target in target_files):
        return
    if not all(seed.exists() for seed in seed_files):
        logger.warning("FAISS seed is incomplete at '%s'; starting with an empty index.", seed_path)
        return
    for seed, target in zip(seed_files, target_files):
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(seed, target)
    logger.info("Bootstrapped FAISS runtime index from '%s'.", seed_path)


def _validate_runtime_config() -> None:
    """Validate critical runtime configuration and fail fast on invalid setup."""
    faiss_index_path = os.getenv("FAISS_INDEX_PATH") or DEFAULT_FAISS_INDEX_PATH
    faiss_dir = os.path.dirname(faiss_index_path) or "."
    os.makedirs(faiss_dir, exist_ok=True)

    if not os.path.isdir(faiss_dir):
        raise RuntimeError(f"Invalid FAISS index directory: {faiss_dir}")
    _bootstrap_faiss_seed(faiss_index_path)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        index_path = os.getenv("FAISS_INDEX_PATH") or DEFAULT_FAISS_INDEX_PATH
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("eduboost_agent.api.main:app", host="0.0.0.0", port=8000, reload=True)
