"""CLI entrypoint for `tools.evaluation` pipeline."""
...
def cmd_rag(args: argparse.Namespace) -> None:
    """Evaluate retrieval hit rate against a RAG gold dataset."""
    from .eval_rag import evaluate_rag

    print_header("RAG RETRIEVAL EVALUATION")
    evaluate_rag(args.gold_dataset, threshold=args.threshold)


def main() -> None:
    parser = argparse.ArgumentParser(description="Eval runner for tools.evaluation")
    ...
    p_rag = subparsers.add_parser("rag", help="Evaluate RAG retrieval hit rate")
    p_rag.add_argument("--gold-dataset", type=str, required=True)
    p_rag.add_argument("--threshold", type=int, default=80)
